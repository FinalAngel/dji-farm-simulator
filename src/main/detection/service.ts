...
export async function analyzeVideo(
  ...
  const args = [
    ...
    "--min-conf",
    String(opts.minConfidence ?? 0.4),
  ];
  if (opts.modelPath) args.push("--model", opts.modelPath);

  const raw = await runPython(py, args);
  const telemetry =
    telemetryPath && existsSync(telemetryPath) ? parseSrt(telemetryPath) : [];
...
function runPython(py: string, args: string[]): Promise<RawDetection[]> {
  return new Promise((resolve, reject) => {
    ...
    p.on("close", (code) => {
      if (code !== 0)
        return reject(
          new Error(`detector exited ${code}: ${err.slice(-2000)}`),
        );
      try {
        ...
        reject(
          new Error(
            `could not parse detector output: ${(e as Error).message}\n${out.slice(0, 500)}`,
          ),
        );
      }
    });
