...
export default function App(): JSX.Element {
  ...
  const reloadFields = useCallback(
    async () => setFields(await api.fields.list()),
    [],
  );
  const reloadFlights = useCallback(
    async () => setFlights(await api.flights.list()),
    [],
  );
  ...
  useEffect(() => {
    ...
    const offInstall = api.onBackendInstallProgress((line) =>
      setInstallLog((l) => [...l, line].slice(-300)),
    );
    return () => {
      offFlight();
      offInstall();
  ...
  useEffect(() => {
    ...
    setParams(
      remembered ?? saved ?? settings?.defaultParams ?? DEFAULT_MISSION_PARAMS,
    );
  ...
  const onMapClick = (ll: LngLat): void => {
    if (drawing) setDraft((d) => [...d, ll]);
  };
  ...
  const onExport = (formats: ExportFormat[], chooseDir: boolean): void => {
    ...
    guard(async () =>
      setExports(
        await api.mission.exportMission(selectedId, params, formats, chooseDir),
      ),
    );
  };
  ...
  const onImportVideo = (): void => {
    ...
    guard(async () => {
      ...
      const flight = await api.flights.analyzeVideo(
        selectedId,
        params,
        video,
        srt ?? undefined,
  ...
  const mapPath =
    view === "flights"
      ? (selectedFlight?.plan.waypoints ?? null)
      : view === "plan"
        ? (plan?.waypoints ?? null)
        : null;
  const mapDetections = view === "flights" ? flightDetections : null;
  const mapSelected =
    view === "flights" ? (selectedFlight?.fieldId ?? null) : selectedId;

  return (
    <div className="app">
      <div className="topbar">
        <div className="brand">
          DJI&nbsp;<span>Farm</span>&nbsp;Simulator
        </div>
        ...
        >
          ...
          {settings ? getDrone(settings.droneId).name : "Lito X1"}
        </div>
        {backend && (
          <div
      ...
      <div className="body">
        <div className="sidebar">
          ...
          {view === "plan" && (
            <PlanView
              ...
              onExport={onExport}
              onReveal={api.system.revealPath}
          ...
          {view === "flights" && (
            <FlightsView
              ...
              detections={flightDetections}
              onSelect={(id) => setSelectedFlightId(id || null)}
        ...
        <div className="map-wrap">
          <MapView
            ...
            basemap={basemap}
            onSelectField={(id) => {
              if (!drawing && (view === "fields" || view === "plan"))
                setSelectedId(id);
            }}
